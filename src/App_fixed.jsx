...
const USE_FALLBACK = true // Ativar dados simulados quando proxy não estiver disponível
...
function App() {
  ...
  const generateFallbackData = () => {
    const colors = ['red', 'black', 'red', 'black', 'red', 'black', 'black', 'red', 'black', 'red']
    const rolls = [1, 8, 12, 4, 7, 13, 10, 5, 11, 14]
    const games = []
    ...
    return {
      ...
      next_prediction: {
        color: 'black',
        confidence: 75,
        reason: 'Análise de padrões (modo offline)'
      }
    }
  }
  ...
  const fetchLiveData = async () => {
    try {
      ...
      if (USE_FALLBACK) {
        ...
        setLiveData(fallbackData.games)
        setNextPrediction(fallbackData.next_prediction)
        setLastUpdate(new Date().toLocaleTimeString('pt-BR'))
        setConnectionStatus('connected')
        setIsConnected(true)
  ...
  const generateSignals = async () => {
    ...
    try {
      ...
      if (USE_FALLBACK) {
        // Gerar sinais simulados como fallback
        const fallbackSignals = generateFallbackSignals()
        setSignalsList(fallbackSignals)
      } else {
  ...
  // Função para gerar sinais simulados
  const generateFallbackSignals = () => {
    const colors = ['red', 'black', 'red', 'black', 'red', 'black', 'red', 'black']
    const confidences = [77, 65, 82, 58, 73, 69, 85, 61]
    const signals = []
    
    const now = new Date()
    for (let i = 0; i < 8; i++) {
      const time = new Date(now.getTime() + ((i + 1) * 4 * 60000)) // 4 minutos de intervalo
      signals.push({
        id: `signal_${i}`,
        color: colors[i],
  ...
  const simulateNewGame = async () => {
    try {
      ...
      if (USE_FALLBACK && usingFallback) {
        // Simular novo jogo no modo fallback
        const fallbackData = generateFallbackData()
        setLiveData(fallbackData.games)
        setNextPrediction(fallbackData.next_prediction)
        setLastUpdate(new Date().toLocaleTimeString('pt-BR'))
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-300">
                  Sinais ao vivo e predições inteligentes
                  {usingFallback && <span className="text-yellow-400"> (Modo Demo)</span>}
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    {connectionStatus === 'connected' ? 
                      (usingFallback ? 'Demo (Offline)' : 'Conectado') : 
